...
type MockDbOverrides = Partial<{
  getAll: () => TodoRow[]
  getById: () => TodoRow | undefined
  insert: () => SqlRunResult
  update: () => SqlRunResult
  delete: () => SqlRunResult
}>

function makeMockDb(overrides: MockDbOverrides = {}): Database {
  const mockRun = vi.fn<() => SqlRunResult>().mockReturnValue({ changes: 1 })
  const mockAll = vi.fn<() => TodoRow[]>().mockReturnValue([])
  const mockGet = vi.fn<() => TodoRow | undefined>().mockReturnValue(undefined)
...
function makeApp(overrides: MockDbOverrides = {}) {
...
describe('PUT /api/todos/:id', () => {
  const existing: TodoRow = { id: 'abc', title: 'Old title', completed: 0, created_at: 1000 }
