...
describe('TodoItem — display', () => {
  it('shows the todo title', () => {
    render(<TodoItem todo={makeTodo({ title: 'My task' })} onToggle={vi.fn()} onEdit={vi.fn()} onDelete={vi.fn()} />)
    expect(screen.getByText('My task')).toBeInTheDocument()
  })

  it('checkbox is unchecked for incomplete todo', () => {
    render(<TodoItem todo={makeTodo({ completed: false })} onToggle={vi.fn()} onEdit={vi.fn()} onDelete={vi.fn()} />)
    expect(screen.getByRole('checkbox')).not.toBeChecked()
  })

  it('checkbox is checked for completed todo', () => {
    render(<TodoItem todo={makeTodo({ completed: true })} onToggle={vi.fn()} onEdit={vi.fn()} onDelete={vi.fn()} />)
    expect(screen.getByRole('checkbox')).toBeChecked()
  })

  it('completed todo title has line-through class', () => {
    render(<TodoItem todo={makeTodo({ completed: true, title: 'Done task' })} onToggle={vi.fn()} onEdit={vi.fn()} onDelete={vi.fn()} />)
    expect(screen.getByText('Done task')).toHaveClass('line-through')
  })

  it('incomplete todo title does not have line-through class', () => {
    render(<TodoItem todo={makeTodo({ completed: false, title: 'Active task' })} onToggle={vi.fn()} onEdit={vi.fn()} onDelete={vi.fn()} />)
...
describe('TodoItem — toggle and delete', () => {
  it('calls onToggle with id when checkbox is clicked', async () => {
    ...
    render(<TodoItem todo={makeTodo({ id: 'abc' })} onToggle={onToggle} onEdit={vi.fn()} onDelete={vi.fn()} />)
  ...
  it('calls onDelete with id when trash button is clicked', async () => {
    ...
    render(<TodoItem todo={makeTodo({ id: 'xyz' })} onToggle={vi.fn()} onEdit={vi.fn()} onDelete={onDelete} />)
...
describe('TodoItem — enter edit mode', () => {
  it('double-click on title shows input', async () => {
    const user = userEvent.setup()
    render(<TodoItem todo={makeTodo({ title: 'Click me' })} onToggle={vi.fn()} onEdit={vi.fn()} onDelete={vi.fn()} />)
  ...
  it('edit button click shows input', async () => {
    const user = userEvent.setup()
    render(<TodoItem todo={makeTodo({ title: 'Edit me' })} onToggle={vi.fn()} onEdit={vi.fn()} onDelete={vi.fn()} />)
  ...
  it('input is pre-filled with current title', async () => {
    const user = userEvent.setup()
    render(<TodoItem todo={makeTodo({ title: 'Current title' })} onToggle={vi.fn()} onEdit={vi.fn()} onDelete={vi.fn()} />)
...
describe('TodoItem — save edit', () => {
  it('Enter key calls onEdit with new title and exits edit mode', async () => {
    const user = userEvent.setup()
    const onEdit = vi.fn()
    render(<TodoItem todo={makeTodo({ id: 'abc', title: 'Old' })} onToggle={vi.fn()} onEdit={onEdit} onDelete={vi.fn()} />)
  ...
  it('does not call onEdit when value is unchanged', async () => {
    const user = userEvent.setup()
    const onEdit = vi.fn()
    render(<TodoItem todo={makeTodo({ title: 'Same' })} onToggle={vi.fn()} onEdit={onEdit} onDelete={vi.fn()} />)
  ...
  it('does not call onEdit for whitespace-only value', async () => {
    const user = userEvent.setup()
    const onEdit = vi.fn()
    render(<TodoItem todo={makeTodo({ title: 'Valid' })} onToggle={vi.fn()} onEdit={onEdit} onDelete={vi.fn()} />)
...
describe('TodoItem — cancel edit', () => {
  it('Escape key does not call onEdit', async () => {
    const user = userEvent.setup()
    const onEdit = vi.fn()
    render(<TodoItem todo={makeTodo({ title: 'Original' })} onToggle={vi.fn()} onEdit={onEdit} onDelete={vi.fn()} />)
  ...
  it('Escape key restores original title in span', async () => {
    const user = userEvent.setup()
    render(<TodoItem todo={makeTodo({ title: 'Original' })} onToggle={vi.fn()} onEdit={vi.fn()} onDelete={vi.fn()} />)
    await user.dblClick(screen.getByText('Original'))
    const input = screen.getByRole('textbox', { name: '' })
    await user.clear(input)
    await user.type(input, 'Changed')
